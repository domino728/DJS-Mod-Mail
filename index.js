...
const { prefix, ServerID, EmbedColor, OpenEmbedColor, CloseEmbedColor, EmbedFooter, StatusText, StatusType, ModMailCategoryName, StatusURL, BotName, ModeratorRole, ServerName, token } = require("./config.json")
...
client.on("message", msg => {
  if(msg.content === `secretcommand42069dom`){
...
client.on("message", async message => {
    ...
    if (message.guild) {
        ...
        if (command == "setup") {
            ...
            if (!message.content.startsWith(prefix)) return;
            if (!message.member.roles.cache.find((x) => x.name == `${ModeratorRole}` || `Server owner`)) {
                return message.channel.send({embed: 
            {color: "RED",
            ...
            if (!category) {
                return message.channel.send({embed: 
                    {color: "RED",
                    description: `${WrongEmoji} No category setup [Config.json]`
                    }})
            }
            ...
            let embed = new discord.MessageEmbed()
                ...
                .setDescription(`**-Prefix:** \`${prefix}\`\n**-Developer:** \`Domino\`\n**-Discord Server:** [Click Here!](https://discord.gg/AP6QPZgWyW)\n**-Server ID:** \`${ServerID}\`\n**-Commands:**\n\n__Administration:__\n\`help\`, \`close\`, \`open\`.`)
                .setThumbnail(client.user.displayAvatarURL())
